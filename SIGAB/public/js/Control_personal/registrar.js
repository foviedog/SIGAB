...
function obtenerIdiomas() {
    ...
    $.each(inputs, function(key, input) {
        // Por cada valor contenido en el array de JQuerry se realiza una función
...
function evtParticipaciones() {
    $("#participaciones-ref").on("click", function(e) {
        ...
        if (!personalForm[0].checkValidity()) {
            // Valida el form antes de que se proceda a la siguiente página para evitar que se envíen datos incorrectos
            $("#registrar-btn").trigger("click"); // Fuerza el envío de datos en el form para que realice la validación automática de los campos
        } else {
            // En caso de que se hayan completado los campos de manera correcta procede a mostrar la información de participaciones
...
function evtAgregarIdiomas() {
    $("#agregar-btn").on("click", function() {
        //El evento se activa con un click en el botón de agregar idioma
        coleccionIdiomas = obtenerIdiomas(); // Se llena el arreglo global de idiomas
        if (coleccionIdiomas.includes("")) {
            // Se verifica si en dicho arreglo existe algún valor vacío
            ...
            //En caso de que no hayan campos vacíos:
            $("#lista-idiomas").append(
                //Se agrega una fila a la tabla que contiene los inpit de idiomas, y dicho inputo contiene un id único que corresponde al orden en el que fue agregado
                '<tr id="row' +
                    i +
                    '" class="idiomaRow"><td><input type="text" name="name[]" placeholder="Nombre de idioma" class="form-control idioma" required/></td><td><button type="button" name="eliminar-idioma" id="' +
                    i +
...
function evtEliminarIdiomas() {
    $(document).on("click", ".eliminar-idioma-btn", function() {
        // Se activa el evento al momento que se le da click a elimianr un idioma
...
function evtSubmitForm() {
    $("#personal-form").on("submit", function() {
        //La función ocurren en el momento en el que se hace submit
